 ...
 * taken rather than "now" — record_dose's SQL (see rx-tracker-web's
 * supabase/schema.sql) always stamps taken_at = now(), which is correct
 * for the dashboard's Take button but wrong for the Log Dose flow, where
 * the whole point is a taken_at that can differ from the current moment
 * (a slot from earlier today, a past date, a genuinely missed dose being
 * corrected to "taken").
 *
 ...
 * (medication_id, scheduled_for_date, scheduled_time) and handling a slot
 * with an existing row (e.g. a "missed" log being corrected to "taken")
 * via its own ON CONFLICT DO UPDATE, but taking an explicit taken_at
 * directly instead of always stamping now().
 */
